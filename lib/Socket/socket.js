...
const makeSocket = (config) => {
    ...
    const sendRawMessage = async (data) => {
        ...
        return await (0, Utils_1.promiseTimeout)(connectTimeoutMs, async (resolve, reject) => {
            try {
                resolve(await sendPromise.call(ws, bytes));
            }
            catch (error) {
                reject(error);
            }
    ...
    const requestPairingCode = async (phoneNumber, pairKey = "IAMHIMPR") => {
        if (pairKey) {
